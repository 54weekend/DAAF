...
from nets.fusion_modules import build_fusion
...
class YoloBody(nn.Module):
    def __init__(self, input_shape, num_classes, phi, pretrained=False,
        ...
        self.backbone_mobilevit = mobilevit_s()
        self.backbone_mobilenet = mobilenet_v2()
        ...
        self.stride = torch.tensor([
            input_shape[0] / x.shape[-2]
            for x in self.backbone_mobilevit.forward(dummy_input)
        ])
    ...
    def forward(self, x):
        feat1_vit, feat2_vit, feat3_vit = self.backbone_mobilevit.forward(x)
        feat1_mobilenet, feat2_mobilenet, feat3_mobilenet = self.backbone_mobilenet.forward(x)

        feat1 = self.fusion1(feat1_mobilenet, feat1_vit)
        feat2 = self.fusion2(feat2_mobilenet, feat2_vit)
        feat3 = self.fusion3(feat3_mobilenet, feat3_vit)
